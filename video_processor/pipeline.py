...
def run_analysis(video_path: str, analysis_types: List[str], twelvelabs_video_id: str = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        twelvelabs_video_id: Video ID from TwelveLabs (required for multimodal analysis)
        ...
        if analysis_type == "multimodal":
            if not twelvelabs_video_id:
                results[analysis_type] = {
                    ...
                    "status": "error", 
                    "error": "TwelveLabs video ID required for multimodal analysis"
                }
            else:
                results[analysis_type] = analyze_multimodal(video_path, twelvelabs_video_id)
...
def get_analysis_descriptions() -> Dict[str, str]:
    ...
    """
    return {
        "multimodal": "Comprehensive video analysis using TwelveLabs AI - fast external processing",
